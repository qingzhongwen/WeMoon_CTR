...
class DNN(Layer):
    ...
    def __init__(self, hidden_units, activation='selu', l2_reg=0.00001, dropout_rate=0, use_bn=True, seed=1024, **kwargs):
        self.hidden_units = hidden_units
        self.activation = activation
        self.dropout_rate = dropout_rate
        self.seed = seed
    ...
    def build(self, input_shape):
        ...
        self.dropout_layers = [tf.keras.layers.AlphaDropout(self.dropout_rate, seed=self.seed + i) for i in
...
class PredictionLayer(Layer):
    ...
    def __init__(self, task='binary', use_bias=True, **kwargs):
        if task not in ["binary"]:
            raise ValueError("task must be binary")
        self.task = task
